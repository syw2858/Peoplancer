...
const FreeReviewAvg = ({ review, fre_rv_target }) => {
  ...
  const getAvg = async () => {
    try {
      const countResponse = await axios.get(
        `http://localhost:8080/frereview/cnt?fre_rv_target=${fre_rv_target}`
      );
      const totalCount = countResponse.data;
      setCount(totalCount);

      const totalResponse = await axios.get(
        `http://localhost:8080/frereview/tot?fre_rv_target=${fre_rv_target}`
      );
